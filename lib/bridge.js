...
const {validator, ActionClient} = require('rclnodejs');
...
class Bridge extends EventEmitter {
  ...
  _rebuildOpMap() {
    ...
    this._registerOpMap('send_action_goal', async (command) => { // FIXME: very basic ROS2 action client
      let actionName = command.actionName;
      let client = new ActionClient(
        this._resourceProvider._node,
        command.type,
        command.actionName
      )

      await client.waitForServer();
      const goal = await client.sendGoal(command.goal, (feedback) => console.log('feedback', feedback));
      const result = await goal.getResult();
